...
def dictItem2List(dict_list, key, unique=True, sort_list=True):
    "extract an item from each dictionary in a list"
    v = []
    for d in dict_list:
        v.append(d[key])
...
def empty_dictlist_variables(dictlist):
    '''  For each key, check whether it is empty in all records
    '''
    ...
    # print(keys)
    for k in keys:
        values = [i for i in list(map(itemgetter(k), dictlist)) if i]
        if len(values) == 0:
            empty_keys.append(k)
...
def dictlist_missing_values(dictlist, keys):
    '''  For each key, check whether it is empty in all records
    '''
    missing_values_in = []
    # print(keys)
    for k in keys:
        column = list(map(itemgetter(k), dictlist))
        values = [i for i in column if i]
        if len(values) < len(column):
            missing_values_in.append(k)

    return(missing_values_in)
